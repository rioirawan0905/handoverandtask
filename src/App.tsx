...
export default function App() {
  ...
  const [emailServiceProvider, setEmailServiceProvider] = useState<"backend" | "emailjs">(() => {
    return (localStorage.getItem("handover_email_provider") as "backend" | "emailjs") || "backend";
  });
  const [emailJsServiceId, setEmailJsServiceId] = useState<string>(() => {
    return localStorage.getItem("handover_emailjs_service_id") || "";
  });
  const [emailJsTemplateId, setEmailJsTemplateId] = useState<string>(() => {
    return localStorage.getItem("handover_emailjs_template_id") || "";
  });
  const [emailJsPublicKey, setEmailJsPublicKey] = useState<string>(() => {
    return localStorage.getItem("handover_emailjs_public_key") || "";
  });
  const [emailJsTestStatus, setEmailJsTestStatus] = useState<{ status: "idle" | "testing" | "success" | "error"; message?: string }>({ status: "idle" });

  const handleTestEmailJsConnection = async (testRecipient: string) => {
    if (!emailJsServiceId.trim() || !emailJsTemplateId.trim() || !emailJsPublicKey.trim()) {
      setEmailJsTestStatus({ status: "error", message: "Please fill in all EmailJS credentials (Service ID, Template ID, and Public Key)." });
      addNotification("Please complete all EmailJS configuration fields before testing.", "warning");
      return;
    }
    setEmailJsTestStatus({ status: "testing" });
    try {
      const payload = {
        ...
        template_params: {
          to_email: testRecipient || "yesaya.rio@gmail.com",
          recipients: testRecipient || "yesaya.rio@gmail.com",
          workspace_name: "Test Workspace",
          workspaceName: "Test Workspace",
          outgoing_lead: "System Test",
          outgoingLead: "System Test",
          incoming_lead: "Active Operator",
          incomingLead: "Active Operator",
          log_text: "This is a direct diagnostic integration test email dispatched from your Handover Application running on your static Cloudflare Pages. This verifies that your service_id, template_id, and user_id are correctly configured and mapped client-side.",
          logText: "This is a direct diagnostic integration test email dispatched from your Handover Application running on your static Cloudflare Pages. This verifies that your service_id, template_id, and user_id are correctly configured and mapped client-side.",
          date: new Date().toLocaleDateString(),
          tasks_count: "1",
          tasksCount: "1",
          backlog_count: "0",
          backlogCount: "0",
          signed_off_by: "Integration Tester",
          signedOffBy: "Integration Tester",
          tasks_list: "[✓] Direct Integration Connection Works Fine!",
          backlog_list: "None"
        }
      };
      ...
      setEmailJsTestStatus({ status: "success", message: "Test email successfully sent!" });
      addNotification("EmailJS Test email successfully sent! Check your inbox.", "success");
    } catch (err: any) {
      console.error("EmailJS test error:", err);
      setEmailJsTestStatus({ status: "error", message: err.message || "Failed to send through EmailJS." });
      addNotification(`EmailJS integration check failed: ${err.message}`, "warning");
    }
  };
  ...
  const handleConfirmSignOffSubmit = async () => {
    ...
    if (signoffEmailOverride.trim()) {
      if (emailServiceProvider === "emailjs") {
        try {
          const payload = {
            service_id: emailJsServiceId.trim(),
            template_id: emailJsTemplateId.trim(),
            user_id: emailJsPublicKey.trim(),
            template_params: {
              to_email: signoffEmailOverride,
              recipients: signoffEmailOverride,
              workspace_name: workspaceName,
              workspaceName: workspaceName,
              outgoing_lead: newHistoryItem.outgoingLead,
              outgoingLead: newHistoryItem.outgoingLead,
              incoming_lead: newHistoryItem.incomingLead,
              incomingLead: newHistoryItem.incomingLead,
              log_text: newHistoryItem.logText,
              logText: newHistoryItem.logText,
              date: newHistoryItem.date,
              tasks_count: newHistoryItem.tasksCount,
              tasksCount: newHistoryItem.tasksCount,
              backlog_count: newHistoryItem.backlogCount,
          ...
          if (!response.ok) {
            throw new Error(totalText || `HTTP Client Error ${response.status}`);
          }

          setSignoffResult({
            status: "success",
            message: `Handover rotation successfully completed and archived! Email dispatched directly via client-side EmailJS Integration.`,
            sentTo: signoffEmailOverride
          });
          addNotification(`📧 Notification sent via EmailJS: ${signoffEmailOverride}`, "success");
        } catch (err: any) {
          console.error("EmailJS dispatch error:", err);
          setSignoffResult({
            status: "error",
            message: `Handover saved to database, but EmailJS report error: ${err.message || "Failed to fetch on client-side"}. Please review your Service ID, Template ID, and Public Key settings.`,
            sentTo: signoffEmailOverride
          });
          addNotification(`⚠️ EmailJS Error: ${err.message || "Endpoint block"}`, "warning");
        }
      } else {
        try {
          ...
          if (data.success) {
            ...
            setSignoffResult({
              status: "error",
              message: `Handover saved & archived, but real SMTP service reported an error: ${data.message || data.error || JSON.stringify(data) || "Connection Block"}.`,
              sentTo: signoffEmailOverride
            });
            addNotification(`⚠️ SMTP block: ${data.message || "Error received"}`, "warning");
          }
        } catch (err: any) {
                ...
                <div className="lg:col-span-6 space-y-4 text-left">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className={`text-[11px] font-bold uppercase tracking-wider ${activeTheme.cardTitleText} font-mono`}>
                      Operational Notification Dispatch Configuration
                    </span>
                  </div>

                  {/* Provider Selector tabs */}
                  <div className="flex rounded-lg bg-slate-100 dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 p-1">
                    <button
                      type="button"
                      onClick={() => {
                        setEmailServiceProvider("backend");
                        localStorage.setItem("handover_email_provider", "backend");
                      }}
                      className={`flex-1 text-center py-2 text-[10px] md:text-xs font-bold rounded-md transition-all cursor-pointer ${
                        emailServiceProvider === "backend"
                    ...
                    >
                      Method A: Cloud Run / Custom Backend API
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                        setEmailServiceProvider("emailjs");
                        localStorage.setItem("handover_email_provider", "emailjs");
                      }}
                      className={`flex-1 text-center py-2 text-[10px] md:text-xs font-bold rounded-md transition-all cursor-pointer ${
                    ...
                    >
                      Method B: EmailJS (Best for Static Host)
                    </button>
                  </div>
                  ...
                  {emailServiceProvider === "backend" ? (
                    ...
                    <div className="space-y-3">
                      <div className={`text-[11px] ${activeTheme.isDark ? 'bg-slate-900 border-slate-800' : 'bg-indigo-50 border-indigo-200'} border rounded-lg p-3 space-y-1.5 text-left leading-relaxed`}>
                        <p className="font-semibold text-indigo-800 text-xs">🚀 EmailJS Client-Side Delivery (Zero Backend Needed!)</p>
                        <p className={`${activeTheme.cardSubText} text-[10px]`}>
                          This method connects your browser directly to the EmailJS API. It does not require any Cloud Run or Node.js server, making it **100% compatible with static deployment plans on Cloudflare Pages**!
                        </p>
                        <p className={`${activeTheme.cardSubText} text-[10px]`}>
                          Sign up at <a href="https://www.emailjs.com/" target="_blank" rel="noreferrer" className="text-indigo-600 font-bold underline">emailjs.com</a>, create a Gmail connection service, draft an email template, and enter the details below.
                        </p>
                      </div>

                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div className="flex flex-col gap-1">
                          <label className={`text-[9px] uppercase font-bold tracking-wider ${activeTheme.cardSubText} font-mono`}>Service ID</label>
                          <input
                            type="text"
                            ...
                            onChange={(e) => {
                              ...
                              localStorage.setItem("handover_emailjs_service_id", val);
                            }}
                            className={`${activeTheme.inputBg} border rounded px-2.5 py-1.5 text-xs focus:ring-1 focus:ring-indigo-400 outline-none w-full font-mono shadow-xs`}
                          />
                        </div>

                        <div className="flex flex-col gap-1">
                          <label className={`text-[9px] uppercase font-bold tracking-wider ${activeTheme.cardSubText} font-mono`}>Template ID</label>
                          <input
                            type="text"
                            ...
                            onChange={(e) => {
                              ...
                              localStorage.setItem("handover_emailjs_template_id", val);
                            }}
                            className={`${activeTheme.inputBg} border rounded px-2.5 py-1.5 text-xs focus:ring-1 focus:ring-indigo-400 outline-none w-full font-mono shadow-xs`}
                          />
                        </div>
                        ...
                        <div className="flex flex-col gap-1">
                          <label className={`text-[9px] uppercase font-bold tracking-wider ${activeTheme.cardSubText} font-mono`}>Public Key</label>
                          <input
                            type="text"
                            ...
                            onChange={(e) => {
                              ...
                              localStorage.setItem("handover_emailjs_public_key", val);
                            }}
                            className={`${activeTheme.inputBg} border rounded px-2.5 py-1.5 text-xs focus:ring-1 focus:ring-indigo-400 outline-none w-full font-mono shadow-xs`}
                          />
                        </div>
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          type="button"
                          onClick={() => handleTestEmailJsConnection("yesaya.rio@gmail.com")}
                          disabled={emailJsTestStatus.status === "testing"}
                          className={`flex-1 py-1.5 text-white rounded text-xs font-bold shadow-xs transition-all active:scale-98 cursor-pointer ${
                            emailJsTestStatus.status === "testing" 
                              ? "bg-slate-400 cursor-not-allowed" 
                              : "bg-indigo-650 hover:bg-indigo-700"
                          }`}
                        >
                          {emailJsTestStatus.status === "testing" ? "Sending Test Email..." : "Send Test Email to yesaya.rio@gmail.com"}
                        </button>
                        {(emailJsServiceId || emailJsTemplateId || emailJsPublicKey) && (
                          <button
                            ...
                            onClick={() => {
                              ...
                              setEmailJsPublicKey("");
                              localStorage.removeItem("handover_emailjs_service_id");
                              localStorage.removeItem("handover_emailjs_template_id");
                              localStorage.removeItem("handover_emailjs_public_key");
                              setEmailJsTestStatus({ status: "idle" });
                              addNotification("Cleared EmailJS configurations.", "info");
                            }}
                            className="px-2.5 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs rounded transition-colors border border-slate-300 pointer-events-auto"
                          >
                            Reset
                      ...
                      <div className="text-[10px] text-slate-500 font-medium">
                        🛠️ Available template parameters: <code className="font-mono bg-slate-100 p-0.5 rounded text-indigo-700 text-[9px]">{"{{to_email}}"}</code>, <code className="font-mono bg-slate-100 p-0.5 rounded text-indigo-700 text-[9px]">{"{{workspace_name}}"}</code>, <code className="font-mono bg-slate-100 p-0.5 rounded text-indigo-700 text-[9px]">{"{{outgoing_lead}}"}</code>, <code className="font-mono bg-slate-100 p-0.5 rounded text-indigo-700 text-[9px]">{"{{incoming_lead}}"}</code>, <code className="font-mono bg-slate-100 p-0.5 rounded text-indigo-700 text-[9px]">{"{{log_text}}"}</code>, <code className="font-mono bg-slate-100 p-0.5 rounded text-indigo-700 text-[9px]">{"{{tasks_list}}"}</code>, <code className="font-mono bg-slate-100 p-0.5 rounded text-indigo-700 text-[9px]">{"{{backlog_list}}"}</code>.
                      </div>

                      {emailJsTestStatus.status !== "idle" && (
                        <div className={`p-3 rounded-lg border text-xs flex items-center gap-2 animate-in slide-in-from-top-1 duration-150 ${
                          emailJsTestStatus.status === "success" 
                            ? "bg-emerald-50 border-emerald-250 text-emerald-850" 
                            : emailJsTestStatus.status === "error" 
                            ? "bg-rose-55 border-rose-200 text-rose-850" 
                            : "bg-blue-50 border-blue-200 text-blue-800"
                        }`}>
                          <span className="font-bold">
                            {emailJsTestStatus.status === "success" ? "✓ SUCCESS:" : emailJsTestStatus.status === "error" ? "⚠ ERROR:" : "⚡ SENDING:"}
                          </span>
                          <span>{emailJsTestStatus.message}</span>
                        </div>
                      )}
                    </div>
                  )}
